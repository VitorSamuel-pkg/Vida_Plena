...
const pagamentoController = {
  ...
  listarPagamento: async (req, res) => {
    try {
      const { nomeCliente, statusPagamento } = req.query; // Pega o ID_Pagamento da query, se fornecido
      const conditions = {};

      if (nomeCliente) {
        conditions.nomeCliente = {
          [Op.like]: `%${nomeCliente}%`,
        };
      }
        ...
        include: [
          {
            ...
            as: "pagamentoCliente",
            where: {
              nomeCliente: { [Op.like]: `%${nomeCliente}%` || `%%` },
            },
          },
        ],
      });
  ...
  cadastrarPagamento: async (req, res) => {
    try {
      ...
      const tiposValidos = ["Débito", "Crédito", "Pix"];
      if (!tiposValidos.includes(formaPagamento)) {
        return res
          .status(400)
          .send(
            `Erro: Tipo inválido. Tipos aceitos: ${tiposValidos.join(", ")}`
          );
      }
      ...
      if (!pedido || !cliente) {
        ...
        return res
          .status(404)
          .json({ message: "Pedido ou cliente não encontrado!" });
      }
    ...
    try {
      ...
      if (pagamento.statusPagamento === "cancelado") {
        return res.status(400).json({
          message: "Pagamento já está cancelado e não pode ser alterado.",
        });
      }
